...
c.norm1 <- rnorm(n,0,.5)
c.norm2 <- rnorm(n,0,0.25)
c.norm3 <- rnorm(n,0,0.25)
amp <- 2
...
# -----------------------------------------------------------------------------
price <- arima.sim(n = n, list(ar = c(0.8897, -0.4858), ma = c(0.279, 0.2488)), sd = sqrt(0.125))
mean(price)
sd(price)
hist(price)
plot(price)
...
sim_df <- add_column(sim_df, price) %>% 
  mutate_at(vars(price), percent_rank)
